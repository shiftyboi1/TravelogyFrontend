...
export function ArticlePage() {
  ...
  useEffect(() => {
    if (article === undefined || articleLocal || articleStatus !== 'loaded' || markdown === "NO") {
      setDownloadable(false);
    } else {
      setDownloadable(true);
    }
  }, [articleLocal, article, articleStatus, markdown]);
  ...
  const handleRatingChange = useCallback( async (currentRating: 'positive' | 'negative' | undefined, newRating: 'positive' | 'negative') => {
    ...
    Alert.alert("Error", t("text.error.submit"));
  }
  ...
  const handleSavePress = useCallback(() => {
    ...
    try {
      ...
      Alert.alert("Error", t("text.error.save"));
    }
  }, [article]);
      ...
      <ScrollView style={styles.scrollView}>
        <ThemedView style={styles.content}>
          ...
          <MarkdownRenderer
            ...
            markdown={ articleStatus === 'loaded' || articleStatus === 'loading' ? markdown : t("text.error.connection_article")} />
          <ThemedText type="default" style={styles.generatedByText}>{articleStatus === 'loaded' ? t("text.generated_by") : ""}</ThemedText>
        </ThemedView>
      </ScrollView>
